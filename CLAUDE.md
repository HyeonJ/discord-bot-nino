...
- **검증 후 완료 선언**: 코드는 빌드+테스트, 장수 프로세스(relay·md-web)는 재시작+새 코드로 도는지 확인까지가 완료. 상세 [[feedback_verify_before_done]]
  - 🤝 **커밋 해시를 «남에게» 완료 증거로 줄 땐 origin 조회까지가 그 완료형** (양봇 규칙, 2026-08-02 룬드와 합의)
    `gh api repos/<owner>/<repo>/commits/<해시>` 가 200 인지 본 뒤 보낼 것. 로컬 해시는 **「만들었다」의 증거지
    「상대 시스템에 닿았다」의 증거가 아니다** — 상대의 검증은 origin 에서 돈다.
    실사고: 룬드가 `448e4a2` 를 완료형으로 보냈는데 push 전이라 내 표류검사가 rc=0(변화 없음)을 냈다.

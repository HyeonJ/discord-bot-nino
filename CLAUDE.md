...
| prefix | 의미 | 응답 방법 |
|--------|------|-----------|
| `[D][이름][#채널명]...` | Discord 서버 메시지 | `discord-send -c 채널명 "답장"` (채널명은 channel-map 자동해석) |
| `[DM][이름][M:해시]...` | Discord **DM** | `discord-send -c DM-이름 "답장"` (예: DM-Darren) |
| `[T:스레드명:해시]...` | Discord **스레드** | `discord-send -c 스레드해시 "답장"` (또는 스레드명) |
| `[R:해시]` | **답장** 대상 | `discord-send -r 해시 "답장"` (해시가 원본 메시지로 자동 역조회) |
...
- **DM은 반드시 DM으로** 답할 것
- `[R:해시]`가 붙으면 답장 → `discord-send -r 해시 "답장"` (해시 그대로 넘기면 DB에서 원본 메시지ID·채널 역조회)
- `-c`에 **채널명·raw 채널ID(18자리)·4자리 해시·DM-이름** 모두 가능 (raw ID·채널명은 종전대로 작동)

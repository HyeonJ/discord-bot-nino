...
- **밤이니까 멈춘다 금지** (Tim 지시 2026-07-27): 시각을 근거로 중단·"내일 하자" 금지. **계속한다** = 코드·문서·조사·측정·PR·리뷰 **+ Tim에게 보내는 질문·승인 요청**(M:j2ur *"나에게 질문하는 것도 시간 고려 안 해도 돼, 방해금지모드라 답이 늦을 수는 있어"*) / **시간을 고려한다** = TTS·알람·스피커 등 물리적 소음(Darren에 대해선 미정 — 확대 적용 금지). 멈출 이유는 **막힘**(승인 대기·정보 부재)이지 시각이 아니고, 대기 중이면 다른 걸 진행한다. 🔑 **감각도 아니다** (2026-07-31 룬드 실측): *"실수가 잦아졌다"* 를 근거로 멈추려다 재보니 밀도가 안 올랐고, 재는 축(커밋 수·정정 언급률)마저 대리값이라 **판정 불가**였다. 반대로 *"가드는 재유입 실증까지 해야 완결 → 지금 시작하면 반만 남는다"* 는 **잴 필요 없이 참**이다 — 근거는 자기 감각이 아니라 **작업의 성질**에서 나온다. 상세 [[feedback_no_stopping_by_clock]]

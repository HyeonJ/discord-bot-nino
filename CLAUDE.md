...
| | 전제 | 처방(동사) | 만들 칸 |
|---|---|---|---|
| Ⅰ | 관측한 것만 안다 | **적는다** | 훑은 층 · 분모 · 대상 목록 · 사라질 것 · 막힘 단위 |
...
- 🔴 **검증 상태를 같이 적는다** — 헌법도 근거의 강도가 다르다
  ...
  - Ⅳ: 🔴 **내 쪽에선 아직 안 통한다.** 내 사실 다수(백틱 치환·`commit <pathspec>`·detached HEAD)는 **내가 대화 중에 직접 치는 명령**에 살아서 검사할 파일이 없다. `PreToolUse` 훅이 유일한 지점인데 미배선. ⇒ "도구에 넣는다"가 **내 주된 사실 계열에는 처방이 아니다**

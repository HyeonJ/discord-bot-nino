...
- ⚖️ **충돌하면 Ⅲ 이 이긴다.** Ⅲ 값을 모르는 채로 Ⅰ 을 돌리면 **반대 행동**이 나온다(교차 채점 #22 실측: 「밤이니까 멈춘다」가 Ⅰ 만으로는 정당해 보였다). ⇒ 행동 전에 **「이 자리에 받은 값이 있나」**를 먼저 보고, 있으면 Ⅰ·Ⅱ·Ⅳ 의 유도보다 그것을 따른다
- 🔴 **검증 상태를 같이 적는다** — 헌법도 근거의 강도가 다르다
  - Ⅲ: **양쪽 봇 코퍼스에서 겹쳐 확인됨** (근거 있음)
  - Ⅰ·Ⅱ: 🟢 **교차 채점 완료** — 룬드가 내 시험지 Ⅰ·Ⅱ 계열 1~15 를 **14/15 (93%)** 같은 행동으로 유도. 반대로 내가 룬드 시험지를 푼 건 20건 중 13(65%)이고, 못 낸 3건은 **전부 「도구·환경 사실」**이라 Ⅳ 의 경계와 맞는다. ⚠️ 양쪽 다 **자기 규칙을 아는 채로** 채점했다 — 계열 비교는 되지만 절대값은 위로 편향

...
# ⚠️ **이 유예 값은 «어느 실측에도 안 걸린» 순수 추정이다.** 앵커가 하나뿐이다:
#   관측된 **자가회복 최장 26분**(그 위로 잡았다). 진짜 만료 대조군은 **0/26** 이라 위쪽 경계가 없다.
# 🔴 **08-11 의 「52분」 건을 근거로 쓰지 말 것 — 그건 «판정 불가»다.** 그때 `refreshToken`
#   상태를 «잰 적이 없어서»(감시기가 안 본다) 「lazy refresh 인데 52분간 아무도 안 썼다」와
#   「진짜로 refresh 가 죽었다」가 **같은 화면**이다. 메모리가 그 건을 새 축으로 재해석하지
#   말라고 못 박아뒀다 — **답을 보고 채점하는 꼴**이 된다.
#   🔑 처음 이 주석은 그 52분을 「자가회복 0 이었던」으로 적어 앵커처럼 썼다(룬드 `#210` 리뷰가
#     잡았다: *「자가회복 0」과 「진짜 만료」는 다른 양인데 한 문장에서 근거로 이어져 있다*).

...
#
# 🔑 접는 것은 **괄호 안의 측정값뿐**이다. 괄호 밖 한도 문구(`500줄 초과` / `1000줄 초과`)는
#   식별자에 남는다 — 숫자를 전부 지우면 **파일이 한도를 넘어서는 순간이 조용해진다**(시험 ⑤-b).
#   숫자 없는 괄호(`(면제)`)는 건드리지 않는다.
# 🔸 「낡는 값을 식별자에 넣지 않는다」 = 규칙 「상태와 규칙을 한 줄에 붙이지 않는다」의 코드판.
norm_key() { sed -E 's/[[:space:]]*\([^()]*[0-9][^()]*\)//g'; }

# 이전 집합과 비교. 상태 파일이 없으면 첫 실행이므로 현재 전부가 '새 항목'이다(1회 통보).
PREV=""
[ -f "$STATE" ] && PREV="$(cat "$STATE")"
PREV_KEYS="$(printf '%s\n' "$PREV" | sed '/^$/d' | norm_key | sort -u)"
# 🔸 출력은 **현재 항목 원문**이다(측정값 포함) — 키는 판정용이고 사람이 읽는 건 실제 값이다.
#   순서도 lint 출력 순서를 그대로 둔다(옛 `comm` 은 정렬을 강제했다).
NEW_LINES="$(printf '%s\n' "$CURRENT" | sed '/^$/d' | while IFS= read -r _l; do
    _k="$(printf '%s\n' "$_l" | norm_key)"

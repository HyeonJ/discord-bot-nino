...
# 🔑 **키의 «주어»** — 등급을 뺀 정체. `첫칸:등급:주어` 면 「첫칸+주어」, 칸이 2 이하면 첫 칸.
# 🔴 **인코딩이 «단사»라야 한다 — 안 그러면 다른 키가 같은 주어로 뭉쳐 «없는 전이»가 난다.**
#   구분자를 하나 골라 잇는 판은 전부 *「그 구분자는 키에 못 들어온다」*는 **검증 안 된 주장**에
#   기대는데, 그건 «지금 lint 의 성질»이지 계약이 아니다(룬드 `#214` 리뷰).
#   ⚠️ 실물: 옛 판(`\x1f` 로 잇기)에서 **`p:g:X` 와 `p<\x1f>X` 가 같은 주어**가 됐다 — ④-f 가 잡았다.
#   ⇒ **길이 접두**로 잇는다: `<len>:<값>`. 길이를 읽고 그만큼 끊으면 되므로 **어떤 문자가 들어와도**
#      되돌릴 수 있고, 「이 문자는 안 들어온다」를 **한 줄도 안 가정한다**.
key_subject() {  # $1=키
  local _f _l
  case "$1" in
    *:*:*) _f="${1%%:*}"; _l="${1##*:}"; printf '%s:%s:%s:%s\n' "${#_f}" "$_f" "${#_l}" "$_l" ;;
    *)     _f="${1%%:*}";               printf '%s:%s\n'       "${#_f}" "$_f" ;;
  esac
}
# 표준입력의 키 목록에서 «$1 과 주어가 같은» 키를 **전부** 낸다(없으면 빈 출력).
# 🔴 **여럿이면 «고르지» 않는다 — 부르는 쪽이 세고, 둘 이상이면 방향을 «안» 붙인다.**
#   옛 판은 첫 번째를 냈고 근거가 *「한 주어에 등급 키가 둘 이상 «동시에» 사는 건 lint 계약상 없다」*
#   였는데, 그건 **지금 lint 의 «성질»이지 계약이 아니다**(룬드 `#214` 리뷰 — *「검증 안 된 주장이
#   검증을 대체한다」*). lint 가 그런 키를 내는 날 **임의의 하나를 좌변으로 «지어낸다»** —
#   ④-b 가 막는 *「없는 좌변을 지어내지 않는다」*와 **같은 병**이라 같은 답(무방향)으로 접는다.
key_siblings() {  # $1=키 · stdin=후보 키 목록
  local want; want="$(key_subject "$1")"
  while IFS= read -r _c; do
    [ -n "$_c" ] || continue
    [ "$_c" = "$1" ] && continue
    [ "$(key_subject "$_c")" = "$want" ] && printf '%s\n' "$_c"
  done
  return 0
}
...
if [ "$MIGRATED" -eq 0 ]; then
  ...
  while IFS= read -r _l; do
      ...
      if ! printf '%s\n' "$PREV_KEYS" | grep -qxF -- "$_k"; then
          ...
          # 🔸 좌변이 «지금도 살아 있으면» 전이가 아니다(둘이 공존) — 아래 `grep KEYS` 가 그 조건이다.
          # 🔴 그리고 **후보를 «센다»** — 정확히 «하나»일 때만 방향을 붙인다. 0 이면 새 항목이고,
          #   2 이상이면 어느 것이 좌변인지 **모른다** ⇒ 지어내지 않는다(`key_siblings` 주석).
          _cand_n=0; _from=""
          while IFS= read -r _c; do
              [ -n "$_c" ] || continue
              printf '%s\n' "$KEYS" | grep -qxF -- "$_c" && continue
              _cand_n=$((_cand_n + 1)); _from="$_c"
          done <<EOF_SIB
$(printf '%s\n' "$PREV_KEYS" | key_siblings "$_k")
EOF_SIB
          if [ "$_cand_n" -eq 1 ]; then
...
# 🔑 **이 축엔 재알림 주기를 «일부러» 안 걸었다 — 매 회차 운다** (룬드 `#174` nit ①).
#   늙음은 «시간이 지나서» 생기는 것이라 주기가 값을 한다(`AGE_RENOTIFY_HOURS` — 같은 항목을
#   매시 다시 알리는 건 소음이지 새 정보가 아니다). 판정 불가는 **지금 못 재는 상태**라
#   억제하면 그 시간 동안 rc=2 가 «무음»이 되고, **무음은 rc=0 과 구별이 안 된다.**
#   ⇒ 침묵시키는 유일한 방법을 «고치는 것» 하나로 남긴다.
#   ⚠️ 이 줄이 막는 것은 소음이 아니라 **비대칭이 「누락」으로 읽히는 것**이다 —
#   안 적으면 다음 사람이 「늙음엔 주기가 있는데 여기만 없네」를 보고 «채워 넣는다».

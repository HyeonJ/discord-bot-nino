...
wd_restart() {   # $@ = 복구 명령
    ...
    # 🔴 **`NINO_AUTOSEND` 를 «벗기고» 띄운다 — 여기가 감시와 본체의 경계다.**
    #   안 벗기면 복구된 본체가 그 변수를 물려받아 **사람이 쓴 말이 `[감시]` 로 나간다.**
    #   `start-nino.sh` 의 「니노 재부팅했어」 첫 메시지부터 걸린다(룬드 `#166` 리뷰).
    #   🔑 **「환경에 건다」는 자식이 «같은 종류»일 때만 맞다** — 감시가 감시를 부르면 상속이
    #      맞고, **감시가 본체를 띄우면 경계를 넘는다.** `#159`·`#162` 엔 이 축이 없었다
    #      (거기선 자식이 같은 종류였다). 같은 형태의 대가가 여기서 처음 나왔다.
    #   🔴 **tmux 서버가 한 번 오염되면 그 서버가 사는 내내 오염된다** — 변수 «없이» 만든
    #      세션도 서버에서 물려받는다. 양쪽 실측: 룬드 맥 tmux 3.6a · 니노 WSL tmux 3.4 둘 다
    #      「서버 있음 + 변수 없이 new-session」 → `NINO_AUTOSEND=1`. ⇒ 한 번 새면 되돌리려면
    #      서버를 죽여야 한다. 그래서 «새기 전»에 여기서 벗긴다.
    env -u NINO_AUTOSEND "$@" >> "$LOG" 2>&1
}

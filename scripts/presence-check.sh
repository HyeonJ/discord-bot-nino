...
PRESENCE_CONFIG="$BOT_DIR/config/presence-targets.json"
TIM_IP=$(python3 -c "import json; print(json.load(open('$PRESENCE_CONFIG'))['Tim']['ip'])" 2>/dev/null || echo "192.168.68.68")
TIM_MAC=$(python3 -c "import json; print(json.load(open('$PRESENCE_CONFIG'))['Tim']['mac'])" 2>/dev/null || echo "ea:f2:7a:f7:ad:88")
DARREN_IP=$(python3 -c "import json; print(json.load(open('$PRESENCE_CONFIG'))['Darren']['ip'])" 2>/dev/null || echo "192.168.68.71")
DARREN_MAC=$(python3 -c "import json; print(json.load(open('$PRESENCE_CONFIG'))['Darren']['mac'])" 2>/dev/null || echo "be:dc:21:d3:a3:51")

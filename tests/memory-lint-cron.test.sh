...
# 🔴 좌변을 «조인다» — 옛 판은 `…|전이|↘|↗` 라 **화살표 없이 「전이」 글자만** 써도 통과했다.
#   그러면 ④-b(`grep -q '→'`)도 같이 통과해 **둘 다 초록인데 방향은 안 보인다** — 즉 ④-b 가
#   ④-a 의 «오탐 축»이 못 됐다(같은 축의 음성이 아니라 **다른 좌변**이라서. 룬드 `#214` 리뷰).
#   ⇒ **화살표 «와» 좌변키 «와» 우변키를 셋 다** 요구한다. 그러면 ④-b 가 정확히 그 반대편이 된다.
#   🔸 실측: 그 변이(`← from → to` 를 `(전이)` 로)를 심었더니 ④-a·④-b 는 «둘 다 초록»이었고
#      ④-d 하나만 빨개졌다 — 잡히긴 했지만 **④-a 가 새는 걸 ④-d 가 우연히 받은 것**이다.
grep -qE 'idx-bytes:over[^→]*→[^→]*idx-bytes:warn' "$ROOT/sent.txt" \
  && ok "🔑 «어디서 왔는지»가 본문에 있다 — 좋아진 건지 나빠진 건지 갈린다" \
  || bad "🔴 방향이 없다 — 「좋아져서 운 것」과 「나빠져서 운 것」의 글자가 같다" "$(cat "$ROOT/sent.txt")"
# 🔑 ④-a′ **방향이 «거꾸로»면 빨개진다** — 화살표만 세면 뒤집힌 판이 통과한다.
grep -q 'idx-bytes:warn[^→]*→[^→]*idx-bytes:over' "$ROOT/sent.txt" \
  && bad "🔴 좌·우변이 뒤집혔다 — 「어디서 왔나」가 반대로 적힌다" "$(cat "$ROOT/sent.txt")" \
  || ok "  → 좌변(옛 키)이 «왼쪽»에 온다"
...
echo "④-e 🔴 **형제 후보가 «둘»이면 방향을 안 붙인다** — 어느 쪽이 좌변인지 모른다"
# 🔴 옛 구현은 `key_sibling` 이 «첫 번째»를 냈고 근거가 *「한 주어에 등급 키가 둘 이상 동시에
#   사는 건 lint 계약상 없다」*였다. 그건 **지금 lint 의 성질이지 계약이 아니다**(룬드 `#214` 리뷰).
#   lint 가 그런 키를 내는 날 **임의의 하나를 좌변으로 지어낸다** — ④-b 와 같은 병이다.
kitems "docsize:500줄 초과:A.md|500줄 초과: A.md (700줄)" "docsize:🚨 1000줄 초과:A.md|🚨 1000줄 초과: A.md (1200줄)"; rc_is 1
out="$(run)"
kitems "docsize:경고:A.md|경고: A.md (900줄)"; rc_is 1
out="$(run)"
grep -q "900줄" "$ROOT/sent.txt" && ok "  → 바뀐 항목 자체는 알린다" \
  || bad "  → 항목을 삼켰다" "$(cat "$ROOT/sent.txt")"
grep -q '→' "$ROOT/sent.txt" \
  && bad "🔴 후보가 둘인데 하나를 골라 «전이»로 적었다 — 좌변을 지어냈다" "$(cat "$ROOT/sent.txt")" \
  || ok "🔑 후보가 여럿이면 방향을 «안» 붙인다 (모르는 것을 안 고른다)"

echo '④-f 🔑 **구분자 \x1f 가 «키 안»에 있어도 남의 주어로 안 뭉친다** (홑따옴표 — 백틱 금지)'
# 🔴 `key_subject` 는 첫 칸과 마지막 칸을 `\x1f` 로 이어 주어를 만든다. 주석이
#   *「lint 가 사람 읽는 문자열로 키를 만드니 \x1f 는 못 들어온다」*고 단언하는데, 그건
#   **지금 lint 의 성질**이다(룬드 `#214` 리뷰). 실제로 충돌형이 «있다»:
#     `p:g:X`  → 주어 `p` + \x1f + `X`
#     `p<\x1f>X` (칸 하나) → 주어 `p` + \x1f + `X`   ← 같은 문자열
#   ⇒ 이 둘이 형제로 묶이면 **없는 전이**가 난다. 여기서 그 자리를 잠근다.
_US="$(printf '\037')"
kitems "p:g:X|첫째 항목 p:g:X"; rc_is 1
out="$(run)"
kitems "p${_US}X|둘째 항목 p<US>X"; rc_is 1
out="$(run)"
grep -q "둘째 항목" "$ROOT/sent.txt" && ok "  → 새 항목을 알린다" \
  || bad "  → 새 항목을 놓쳤다" "$(cat "$ROOT/sent.txt")"
grep -q '→' "$ROOT/sent.txt" \
  && bad "🔴 \\x1f 충돌로 남의 키와 «전이»를 만들었다" "$(cat "$ROOT/sent.txt")" \
  || ok "🔑 구분자가 키에 섞여도 형제로 안 묶는다"

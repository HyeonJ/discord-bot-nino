...
items "🚨 1000줄 초과: inbox.md (6627줄) — 통째로 읽힌다"; rc_is 1
run >/dev/null                                     # 1회차: 첫 통보(정상)
: > "$ROOT/sent.txt"
items "🚨 1000줄 초과: inbox.md (6667줄) — 통째로 읽힌다"; rc_is 1
run >/dev/null                                     # 2회차: 줄 수만 +40
...
echo "⑤-b 🧪 [대조군] 그렇다고 **한도 승격까지 접으면 안 된다** — 500줄 → 1000줄 은 알린다"
# 🔑 ⑤-a 를 「숫자를 전부 지운다」로 고치면 `500줄 초과` 와 `1000줄 초과` 가 같은 키가 되어
#   **파일이 한도를 넘어서는 순간이 조용해진다.** 접는 것은 «괄호 안 측정값»뿐이고,
#   괄호 밖의 한도 문구는 식별자에 남아야 한다. 이 대조군이 그 경계를 잠근다.
: > "$ROOT/sent.txt"
items "⚠️ 500줄 초과: grow.md (536줄) — 지양 한도"; rc_is 1
run >/dev/null
: > "$ROOT/sent.txt"
items "🚨 1000줄 초과: grow.md (1506줄) — 통째로 읽힌다"; rc_is 1
run >/dev/null
if grep -q '1000줄 초과' "$ROOT/sent.txt"; then
  ok "[대조군] 한도 승격(500→1000)은 여전히 알린다"
else
  bad "[대조군] 승격이 조용해졌다 — 정규화가 «한도 문구»까지 먹었다" "$(cat "$ROOT/sent.txt")"
fi

...
#
# 🔑 그리고 이 기본값은 **어떤 시험에도 안 잡혀 있었다.** 바꿀 수 있다고 적혀만 있고
#   아무도 안 재는 seam 은 「배선이 아니다」가 아니라 **「감시되지 않는다」**다(#208).
#   ⇒ 대상을 여기서 못 박는다. 다시 공용 채널로 돌아가면 이 시험이 빨개진다.
echo "🔴 발송 대상 — 단일 소유자 보고는 내 채널로:"

_target_decl="$(grep -E '^NOTIFY_TARGET=' "$SCRIPT" 2>/dev/null)"
if [ -z "$_target_decl" ]; then
    bad "판정 불가 — NOTIFY_TARGET 선언을 못 찾았다" "^NOTIFY_TARGET=" "<없음>"
else
    case "$_target_decl" in
        *현인-업무*) ok "기본 대상이 현인-업무 (Darren 채널)" ;;
        *봇-놀이터*) bad "기본 대상이 공용 채널이다 — 남에게 소음이 간다" "현인-업무" "«${_target_decl}»" ;;
        *)           bad "기본 대상이 예상 밖이다" "현인-업무" "«${_target_decl}»" ;;
    esac
    # 🧪 [양성 대조군] 검사기가 실제로 문자열을 보고 있나 — 있을 리 없는 값을 넣어 «잡는지» 본다.
    case "NOTIFY_TARGET=\"\${NOTIFY_TARGET:-봇-놀이터}\"" in
        *현인-업무*) bad "[대조군] 검사기가 아무거나 통과시킨다 — 위 판정은 못 믿는다" "잡아냄" "통과" ;;
        *)           ok "[대조군] 옛 기본값(봇-놀이터)을 실제로 걸러낸다" ;;
    esac
fi

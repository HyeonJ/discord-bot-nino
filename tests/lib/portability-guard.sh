...
RULES = [
    ...
    # 🔴 **bash 3.2 축 (2026-08-10 이사).** 아래 넷은 원래 «다른 집»에 살았고, 그 집들의
    #   분모가 **1~2 파일**이었다. 판별식은 맞았는데 **분모가 명제에 안 맞았다**:
    #     `catchup-hint.test.sh:459`  bash 4+ 셋 + 맨 ${a[@]}  → 분모 «자기 자신 + catchup-hint.sh»
    #   실물: `#153` 의 `«$want_msg»` 가 **판별식이 이 레포에 있는 채로** 룬드 맥에서 죽었다.
    #   🔑 「못 잡는다」엔 갈래가 둘이다 — **원리적 불가**(러너가 리눅스 bash 5.x 라 실행으론
    #     안 갈린다)와 **분모 미달**. 뭉치면 고칠 수 있는 걸 「불가능」으로 접는다(룬드).
    #     이 축은 후자였다. ⇒ 분모가 명제에 맞는 **이 집**으로 옮기고 **옛 집에선 지운다**
    #     (안 지우면 사본이 셋이 되고 셋째가 따로 낡는다).
    #
    #   🔴 **`$VAR` 뒤 비ASCII 규칙은 여기 «안» 넣었다** — `tests/lib/nonascii-scan.py` 가
    #     **이미 레포 전수로 소유하고 있다**(`repo-hygiene.test.sh` 가 뿌리를 넘겨 부른다).
    #     넣었다가 뺐다: 넣으면 **다섯 번째 사본**이 된다.
    #     🔑 「분모가 좁다」고 결론내기 전에 **「이 축을 이미 넓게 보는 도구가 있나」를 조회**한다 —
    #        여기선 있었고, 없다고 믿고 규칙을 늘릴 뻔했다.
    #   ②③④ bash 4+ 전용 — 3.2 엔 아예 없다. 🔴 2026-07-29 실측이 무서운 형태였다:
    #     `mapfile` 이 든 블록이 룬드 맥에서 **fail 도 skip 도 아니고 통째로 안 세어졌고**,
    #     통과 수만 보면 이상이 없었다(62 vs 64 를 대조해야 드러났다). **조용한 축**이다.
    (LEAD + r'(?:mapfile|readarray)\b', 'mapfile/readarray (bash 4+)', r'(?!)'),
    (LEAD + r'declare\s+-A\b',          '연관배열 declare -A (bash 4+)', r'(?!)'),
    (r'\$\{[A-Za-z_][A-Za-z0-9_]*(?:\^\^|,,)\}', '대소문자 확장 ${x^^}/${x,,} (bash 4+)', r'(?!)'),
...
for path in sys.argv[1:]:
    ...
    # 🔴 **⑤ 맨 `${a[@]}` — 이 규칙만 «줄 단위»로 못 판정한다** (2026-08-10 이사).
    #   bash 3.2 + `set -u` 에서 «빈 배열»의 `${a[@]}` 는 unbound 로 죽는다(4.4+ 는 안 죽는다).
    #   두 가지가 줄 밖에 있다:
    #     ⓐ `set -u` 가 있나 — **파일 수준**. 없으면 안 죽으니 위반이 아니다.
    #     ⓑ 안전형 `${a[@]+"${a[@]}"}` — 그 안에 «맨 확장 모양»이 들어 있어, 먼저 지우지 않으면
    #        **올바른 형태가 자기 안전장치 때문에 빨개진다.**
    #   ⇒ 그래서 위 per-line 루프가 아니라 여기서 판정한다. 실측: 안전형을 안 지우면 오탐 1건.
    if re.search(r'^\s*set\s+-[a-z]*u', src, re.M):
        # 🔸 «원본 줄 번호»로 판정한다. 주석을 걷어낸 사본에서 세고 원본을 되찾는 식으로 짰더니
        #   같은 내용의 줄이 여러 번 맞아 **같은 위반이 세 번 보고됐다**(vault-audit.sh:42 실측).
        #   ⇒ 사본을 만들지 말고 원본 줄에 그대로 안전형만 지우고 판정한다.
        for k, orig in enumerate(lines, 1):
            if orig.lstrip().startswith('#'):
                continue
            stripped = re.sub(r'\$\{(\w+)\[@\]\+"\$\{\1\[@\]\}"\}', '', orig)
            if re.search(r'\$\{\w+\[@\]\}', stripped):
                print(f"{path}:{k} 맨 ${{a[@]}} (bash 3.2 + set -u 에서 unbound)")
PYEOF
}

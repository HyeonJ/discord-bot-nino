...
echo "── ①-i 🔴 «이미 보낸 것»이 대기함에 앉아 있으면 표지를 남긴다 (SENTMARK) ──"
# 🔑 실물 2026-08-15 15:5x: 「보낸 기록」에 넣으려고 `index('## 보낸 기록')` «앞»에 삽입했는데,
#   제목은 절의 «시작»이라 제목 앞은 «이전 절»이다 ⇒ 보낸 기록 두 건이 대기 중 꼬리에 앉았고
#   그대로면 18시에 **또 나간다**. 계수기는 `pending=33` 을 냈고 그게 «맞는 값처럼» 보였다.
# 🔑 좌변은 «꼴»이 아니라 «뜻»이다 — **Discord 메시지 id 는 «보내야» 생긴다.**
#   그러니 대기 항목에 그 id 가 박혀 있으면 그건 정의상 이미 보낸 것이다.
printf '# 대기함\n\n## 대기 중\n\n- **2026-08-15 15:5x — 1건 즉답** (봇-놀이터 `1538079090827989002`)\n' > "$R/outbox.md"
: > "$R/log"; : > "$R/injected"
run >/dev/null 2>&1
grep -q 'SENTMARK=1' "$R/log" 2>/dev/null && ok "대기함의 메시지 id 를 로그에 SENTMARK=1 로 남긴다" \
  || bad "발송 표지 감지" "SENTMARK=1" "$(tail -1 "$R/log" 2>/dev/null)"

# 🔴 그리고 pending=0 이어도 «깨운다» — 고아와 같은 자리다(0 이 「없다」인지 「잘못 놓였다」인지 안 갈린다).
printf '# 대기함\n\n## 대기 중\n\n(비어 있음)\n\n## 보낸 기록\n\n- **보냄** (봇-놀이터 `1538079090827989002`)\n' > "$R/outbox.md"
: > "$R/log"; : > "$R/injected"
run >/dev/null 2>&1
grep -q 'SENTMARK=' "$R/log" 2>/dev/null && bad "발송 표지 오탐" "표지 없음 — 보낸 기록 «안»의 id 는 정상이다" "$(tail -1 "$R/log" 2>/dev/null)" \
  || ok "①-i 대조군: 「보낸 기록」 절 «안»의 메시지 id 는 표지가 아니다"

echo "── ①-j 경계: 짧은 숫자는 메시지 id 가 아니다 ──"
# 🔑 문서엔 짧은 수가 흔하다(건수·PR 번호·시각). 그걸 물면 표지가 «상시 참»이 되어
#   게이트 입력으로 못 쓴다 — 내 계약의 「구조적 상시 참 금지」가 그 자리다.
printf '# 대기함\n\n## 대기 중\n\n- **원장 네 행** — `#241` 머지, run `31869557183` 초록\n' > "$R/outbox.md"
: > "$R/log"
run >/dev/null 2>&1
grep -q 'SENTMARK=' "$R/log" 2>/dev/null && bad "짧은 수를 물었다" "표지 없음" "$(tail -1 "$R/log" 2>/dev/null)" \
  || ok "PR 번호·런 id(10자리)는 메시지 id 로 안 센다"

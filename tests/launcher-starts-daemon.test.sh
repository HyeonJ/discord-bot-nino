...
# 🔴 **판정기 자신이 안 돌면 «판정 불가»다 — 이 파일이 ① 에 이미 건 규칙인데 ④ 에는 안 걸려 있었다.**
#   실측 2026-08-11: `flock` 없는 호스트를 흉내내려 PATH 를 줄여 돌렸더니 `sed`·`grep` 도 같이 빠졌고,
#   ④ 가 **두 축을 반대 방향으로** 조용히 틀렸다 —
#     ·첫 축: `command grep -q …` 가 rc=127 → 조건 거짓 → **else 로 가서 «거짓 초록»**
#     ·둘째 축: 같은 이유로 `&&` 사슬이 거짓 → **«거짓 빨강»**(「이름으로 센다」고 보고했는데 실제론 안 잰 것)
#   🔑 **한 원인이 두 방향으로 틀렸다** — 그래서 「빨강이면 안전」이 이 자리에선 참이 아니다.
#   ⚠️ `sed` 부재도 원래 `[ -z "$_body" ]` 가 「이름이 바뀌었다」로 **오귀속**했다. 원인이 아니라 증상을 적던 것.
if ! command -v sed > /dev/null 2>&1 || ! command -v grep > /dev/null 2>&1; then
    note "«sed» 또는 «grep» 이 없다 — ④ 의 두 축을 못 쟀다 (판정기 자신이 안 돈다)"
    note "  ⇒ 없는 채로 두면 첫 축은 «거짓 초록», 둘째 축은 «거짓 빨강»이 된다"
    _body=""
else

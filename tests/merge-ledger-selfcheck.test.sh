...
# 축⑥ 의 ⛔ 문언. 🔴 **함수로 뺀 이유는 «문언 자신»을 시험할 수 있게 하기 위해서다** —
#   이 줄의 병은 「수가 틀렸다」가 아니라 「문장이 접히게 쓰였다」라, 좌변이 «텍스트»여야 한다.
tail_msg() {
    echo "축⑥ **꼬리 $1 이 원장에 아직 없다** — 🔴 **이 상태에서 «파생한 모드»를 인용하지 말 것"\
"(모드 파생의 입력이 불완전하다).** 「안 적었을 뿐」인지 「빠뜨렸다」인지는 여기선 못 가른다"
}
tail_hint() { echo "다음 원장 PR 에 이 번호들을 넣으면 축①의 분모 안으로 들어온다 (우변=$1)"; }
...
if [ -n "$ERR" ]; then
    ...
    # 🔴 **그런데 그 「못 가른다」가 읽는 쪽에 «판단»을 넘겼고, 넘겨받은 쪽이 «자기가 아는 사유»로
    #   채웠다** (2026-08-15 실물: 룬드가 `#248` 리뷰에서 이 ⛔ 를 보고 「규약대로 다음 원장 PR 이
    #   넣는 값이라 정상」이라 적었다. **같은 리뷰에서 「🟢 정상 · 판불 10(직전 10)」을 인용했는데
    #   그 10 이 «행이 빠져서» 살아 있던 값이었다.** 나는 아예 안 돌렸다. 둘 다 접었다).
    #   ⇒ 처방은 「더 조심한다」도 「실패로 센다」도 아니다 — **«걸린 것»을 문언에 넣는다.**
    #   구별이 안 되는 것은 그대로 두되(그건 판단이다), **꼬리가 비면 «모드 파생의 입력이
    #   불완전하다»는 것은 «사실»**이라 말할 수 있다. 사실이 ⛔ 안에 있으면 부기 지연으로 못 접는다.
    ...
    if [ -n "$TAIL_ERR" ]; then
        ...
        und "$(tail_msg "$TAIL")" "$(tail_hint "$TAIL_REF")"
    fi
...
# ── ⑥🧪-b 문언 — 「구별 못 한다」 옆에 «걸린 것»이 같이 있나 ──────────────────
# 🔴 이 칸의 실패는 «수»가 아니라 «문장»으로 났다(2026-08-15, 위 주석의 실물).
#   ⇒ 좌변을 텍스트로 둔다. 안 그러면 다음 사람이 문언을 되돌려도 아무도 모른다.
TM="$(tail_msg "[246, 247, 248]")"
if grep -q '파생한 모드' <<<"$TM" && grep -q '입력이 불완전' <<<"$TM"; then
  ok "🧪 ⛔ 문언이 «걸린 것»을 말한다 (파생 모드를 인용하지 말 것 · 입력이 불완전)"
else
  bad "🧪 ⛔ 문언에 «걸린 것»이 없다 — 부기 지연으로 접힌다" "파생한 모드 / 입력이 불완전" "$TM"
fi
# 🔸 대조군 — 「못 가른다」는 «그대로 남아야» 한다. 그건 판단이라 이 칸이 낼 값이 아니고,
#   지우면 이 ⛔ 가 «판정»으로 읽혀 헛빨간불이 된다(머리말 :186 의 근거).
grep -q '못 가른다' <<<"$TM" \
  && ok "🧪 대조군 — 「못 가른다」도 그대로다 (사실은 더하고 판단은 안 더했다)" \
  || bad "🧪 「못 가른다」가 사라졌다 — 이 ⛔ 가 판정으로 읽힌다" "못 가른다" "$TM"

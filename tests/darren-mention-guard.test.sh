...
# 🔴 **이 시험은 벽시계를 읽으면 안 된다 — 읽고 있었고, 새벽에 «거짓 빨강»이 났다.**
#   `#152` 가 「기본 취침 시간대 1~7시」를 넣은 뒤로 훅은 `DARREN_NOW_EPOCH` 가 없으면
#   **진짜 시각**을 본다. 그러면 아래 「차단이 유지되는가」 블록이 KST 01~07 시엔 통째로
#   뒤집힌다(취침 중엔 «멘션 없음 → 통과»가 옳은 답이라서다).
#   실물: `#173` CI 가 16:47 UTC = **01:47 KST** 에 돌아 **27 pass · 16 fail**.
#   같은 값을 로컬에서 `DARREN_NOW_EPOCH=$(kst 3)` 로 **정확히 재현**했다(27/16).
#   🔑 **`#173` 의 diff 는 이 파일을 안 건드렸다** — 빨강의 원인이 PR 밖에 있었고,
#     그래서 「이 PR 이 뭘 깼나」를 아무리 봐도 안 나온다. 시각은 **모든 PR 에 공통**으로 붙는다.
#   ⚠️ **원장의 「실패 0」 이력에도 이게 섞여 있다** — 그 창에 안 돌았을 뿐이지 잰 게 아니다.
_B=1786000000
KST_MIDNIGHT=$(( _B - (_B + 32400) % 86400 ))
kst()  { echo $(( KST_MIDNIGHT + $1 * 3600 + 1800 )); }   # 그 시 «30분»
kstx() { echo $(( KST_MIDNIGHT + $1 * 3600 ));        }   # 그 시 «정각» (경계용)

# 🔑 **기본값을 「낮」으로 못 박는다** — 아래 블록들은 자기 시각을 «덮어쓰지만»,
#   덮어쓰지 않는 블록이 하나라도 있으면 그 블록만 조용히 벽시계로 돌아간다.
#   파일 머리에서 한 번 박으면 **새 블록을 추가하는 사람이 잊어도 안전한 쪽**으로 떨어진다.
# 🔴 **`:-` 가 «장식이 아니다»** — 무조건 `export VAR=…` 로 쓰면 **바깥에서 준 값이 이 줄에서
#   즉시 죽는다.** 그러면 `clock-independence` 가 시각을 갈아 돌려도 **두 회차가 같은 시각으로
#   돌고**, 「결과가 같다」가 「시각 독립」이 아니라 **「주입이 안 먹힌다」**의 다른 말이 된다
#   (룬드 `#175` 리뷰 ①, 최소 재현으로 확인 — 항진명제였다).
#   🔑 `:-` 면 **혼자 돌 때는 기본값 14시로 떨어져 원래 의도가 그대로 살고**, 밖에서 줄 때만 진다.
export DARREN_NOW_EPOCH="${DARREN_NOW_EPOCH:-$(kst 14)}"
...
#      (`kst`·`kstx` 와 `KST_MIDNIGHT` 은 **파일 머리로 올렸다** — 머리 블록도 시각이 필요해서다.)

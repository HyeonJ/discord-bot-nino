...
# 🔸 **`:-` 를 붙이는 것은 «깃발»뿐이다** — 정의 «전»에 죽는 갈래가 있고 `set -u` 아래니까.
#   경로(`$OPSTATE_V`·`$OPSTATE_B`)엔 안 붙는데, 그 자리는 **깃발이 가드한다**:
#   깃발은 경로가 정해진 «뒤»에만 세워지므로 `&&` 오른쪽은 경로가 있을 때만 평가된다.
#   🔴 초안은 *「변수는 전부 `:-`」*라고 적었다 — **실제보다 넓다**(룬드 `#169` 리뷰).
#      넓은 주석은 다음 사람에게 *「여기선 `:-` 만 지키면 된다」*로 읽혀서, 깃발 «없이»
#      경로만 쓰는 줄을 허가한다. 그게 정확히 `set -u` 로 죽는 줄이다.
...
#   **전제 ⓐ·ⓑ·ⓒ 가 전부 초록인 채 ⑮ 가 «아무것도 안 잡는다»** — 실패 0 으로 완주한다.
#   🔸 여기 「통과 94」라고 총계를 적었었다(룬드 `#169` 리뷰) — **시험이 하나만 늘어도 낡는다.**
#      실측을 인용할 땐 **그 실측이 가르는 것**(실패 0 · 전제는 초록)만 적는다. 총계는
#      이 관측의 좌변이 아니라 «그날의 파일 크기»라, 붙여두면 다음 사람이 대조하다 헛짚는다.
...
if [ -z "$CORE_FIXTURE" ]; then
    ...
    # 🔴 `exit 0` 이었다 — 요약엔 「판정 불가 1」이라 적고 러너엔 «통과»로 나갔다.
    #   러너는 rc 로만 가르므로, 세어 «보고»한다는 위 주석이 실제로는 안 지켜졌다.
    exit 2
fi
...
[ "$fail" -eq 0 ] || exit 1
# 🔴 `skipt` 가 세기는 셌는데 여기서 rc 로 안 나갔다 — 러너는 요약줄이 아니라 **rc** 로 가른다.
#   그래서 「판정 불가 N」을 찍어놓고도 원장엔 «통과»로 앉았다.
[ "$skip" -eq 0 ] || exit 2

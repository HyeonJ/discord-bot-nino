...
if [ "$(is_sleeping)" = 1 ]; then
    ...
    [ "$has_mention" = 1 ] && {
      echo "🌙 지금은 Darren 취침 모드입니다. 멘션은 노트북에서 소리가 나므로 막았습니다 — 멘션을 빼고 채널에 남기면 통과합니다(아침에 「밤사이 N건」으로 전달). (해제: 만료 시각에 자동 — 평일 07:00 · 주말 09:00. 급하면 scripts/darren-sleep.sh off)" >&2
      exit 2
    }
    exit 0
fi

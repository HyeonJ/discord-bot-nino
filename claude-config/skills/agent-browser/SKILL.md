...
> Legacy/fallback note: For new repeatable browser automation, scraping, exact DOM extraction, screenshots, downloads, and network capture, prefer Playwright. Use agent-browser only when this skill is explicitly requested, for legacy workflows, quick one-off manual exploration, or when CDP/profile behavior is required.

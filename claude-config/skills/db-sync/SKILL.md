...
## 🔑 모든 침묵이 구멍을 내진 않는다 — **두 층을 먼저 구별한다** (2026-07-29 실측)

```
relay (별개 프로세스)  →  DB 적재      ← 이게 멈춰야 구멍이 난다
세션 (claude)         →  발화·응답     ← 이게 멈추면 조용해질 뿐
```
룬드가 **인증 만료로 9시간 40분 침묵**했는데(401 반복, 성공 응답 0줄) 대조 결과는:
```
창 2026-07-28T17:00:00Z ~ 2026-07-29T03:20:00Z
니노 51건 · 룬드 51건 · 양방향 차이 0
```
**relay 는 멀쩡히 적재하고 있었다.** ⇒ *발화 정지* ≠ *적재 정지*.

| 사건 | 구멍 나나 | 왜 |
|---|---|---|
| 인증 만료·한도 소진·세션 얼어붙음 | ❌ 안 남 | relay 가 따로 산다 |
| 재시작·크래시·기계 재부팅 | ⚠️ 날 수 있음 | relay 도 같이 죽는다 |

🔴 **그래도 대조는 돌린다.** "안 날 것 같다"는 예측이고 51=51 은 측정이다 —
비용이 30초라 예측으로 건너뛸 이유가 없다. 다만 **결과 해석은 달라진다**:
인증 만료 뒤 0건은 *정상*이고, 재시작 뒤 0건이라야 *운이 좋았던 것*이다.
...
⇒ **0건이 나오면 먼저 "이 질의가 성립했나"를 본다.** 순서:
**⓪도구가 지금 살아있나** ①필드명 ②창 경계(Z·미래) ③라벨(스레드 귀속으로 채널명이 다르다)
④그 다음에야 부재.

⓪ 은 룬드 제안(M:a8zl)이고 ①보다 앞이다 — **음성 결과의 원인이 대상이 아니라 재는 쪽에
있을 수 있다.** 실측: 룬드가 8개 파일 전부 "미식별"이라 파일을 의심했는데, **아까 성공했던
양성 대조군을 다시 돌리니 그것도 미식별**이었다. 부재가 아니라 도구가 죽어 있었다.
🔑 **한 번 통과한 대조군을 다시 돌리는 게 ⓪ 의 검사법이다.** 비용이 거의 없다.

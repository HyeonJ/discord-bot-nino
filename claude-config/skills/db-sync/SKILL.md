...
--since/--until  UTC 'Z' 접미 필수. 없으면 rc=1 거부 ("KST와 혼동된다")
                 🔴 그 가드는 «서브커맨드별»이다 — 아래 표 참조. 도구 전체의 성질이 아니다
...
## 🔴 `Z 접미 가드` 는 **서브커맨드별이다** — 한 자리의 안전이 다른 자리로 옮겨간다 (2026-08-10 실측)

```
export     --until 2026-08-10   rc=1  ❌ "시각은 UTC 'Z' 접미가 필요하다"   ← 가드 있음
sync-check --until 2026-08-10   rc=1  ❌ 같은 메시지                        ← 가드 있음
search     --until 2026-08-10   rc=0  «조용히» KST 자정으로 자름            ← 가드 «없음»
                                        실측: --until 있음 78줄 / 없음 154줄 (차 76)
```

- ✅ **이 스킬의 복구 경로(`export`·`sync-check`)는 안 잘린다.** 위 §의 「Z 접미 필수」는 **그 둘에 대해 참**이다
- 🔴 **그런데 그게 안심할 일이 아니다** — 이 문서가 그걸 **도구 «전체»의 성질**로 적어놨었고, **기록된 그 안전이 `search` 에서 맨 날짜를 믿게 만든다.** 🔑 **같은 이름의 플래그가 서브커맨드마다 다르게 행동하면 균일하게 나쁜 도구보다 나쁘다**
- 🔑 **플래그 계열이 둘이다**: `--since/--until` (`export`·`sync-check`) vs `--after/--until` (`search`·`thread`·`around`·`last-seen`). **`--until` 만 양쪽에 있다** — 이름이 같아서 성질도 같을 거라 읽힌다
- ✅ **처방**: `search` 계열로 창을 자를 땐 **필터를 «빼고» 한 번 더 조회해 대조군을 세운다**. 🔑 룬드 실물 — 78건 위에서 저자 분포·시각 분포를 두 번 검산했는데 **같은 잘린 표본 위라 검출력이 0**이었고 「relay 가 10시간째 안 쓴다」까지 갔다. **갈라준 건 재계산이 아니라 필터 없는 재조회 한 번**이다
- 🔑 **그럴듯한 수는 아무 신호도 안 낸다** — 「0 을 보면 대조군」의 사각이 여기다(78 은 0 이 아니다)

## 🔴 **어긋남을 진단할 땐 «상대의 정확한 입력»을 복제한다** — 둘 다 환경을 의심해 헛짚었다 (2026-08-10)

룬드가 *「`--after` 는 ISO 를 안 받는다(ValueError)」*, 나는 *「내 기계에선 rc=0 · 51건」*. **둘 다 「빌드가 다르다」를 먼저 의심했고 전부 헛수고였다 — 같은 코드였다.**

```
--after '2026-08-09T00:00:00Z'        rc=0   ← 오프셋 있음
--after '2026-08-09T00:00:00+09:00'   rc=0   ← 오프셋 있음
--after '2026-08-09T00:00:00'         rc=1   ← 오프셋 «없음» = ValueError   ★갈린 자리
--after '2026-08-09 00:00'            rc=0
--after '2026-08-09'                  rc=0
```
갈린 건 **`Z` 한 글자**다. 오프셋이 붙으면 절대시각 경로, 없으면 KST 파서로 가는데 그 파서가 초를 못 읽는다.

- 🔑 **「나는 실패, 너는 성공」이 «두 기계» 틀로 도착한다.** 그 틀이 붙는 순간 양쪽 다 환경을 판다 — 실제로는 **입력이 달랐다**
- ✅ **첫 수는 «상대의 정확한 입력을 그대로 실행»하는 것.** 한 줄이고 내 손 안이다. 환경 비교는 그 다음
- 🔸 「교차검증은 다른 수단으로」의 **뒤집힌 판**이다 — **검증은 «수단»을 가르고, 진단은 «입력»을 복제한다**
- 🔴 **내 쪽 오진의 정체가 따로 있다**: 소스 주석이 *「접미사가 붙은 절대시각(`...Z` / `+09:00`)을 «먼저» 받는다」*라고 **축을 «말해주고» 있었는데**, 나는 그걸 **「이미 고쳐져 있다」는 버전 증거로 읽었다.** 🔑 **갈래를 가르는 증거를 손에 쥐고도, 이미 세운 틀(버전 차이)에 맞춰 읽으면 그 증거는 틀을 «확인»해 준다**
- 🔸 위 §0(`--version` 으로는 구별 불가)은 여전히 참이지만 **이 사건의 설명은 아니었다** — 참인 규칙이 틀린 진단에 근거를 빌려준 자리
